...
namespace main_savitch_10
{
    ...
    {
        if (root_ptr == NULL)//empty list
            return;
        if (root_ptr != NULL)//not empty list
        {
            if (root_ptr->right() != NULL)//not end of list of right side
            {
                bst_remove_max(root_ptr->right(), removed);
            }
            else if (root_ptr->right() == NULL)//end of the list of right side
            {
    ...
    // of the target were removed.
    {
        ...
        {   // Continue looking in the left subtree
            /* STUDENT WORK */
            bst_remove_all(root_ptr->left(), target);//recursively search left subtree
        }
        
        if (target > root_ptr->data( ))
        {   // Continue looking in the right subtree
            /* STUDENT WORK */
            bst_remove_all(root_ptr->right(),target);//recursively search right subtree
        }
        ...
        /* STUDENT WORK */
        bst_remove_all(root_ptr->left(), root_ptr->data());//replace this target with the maximum item in our LEFT subtree
    ...
    // Header file used: bintree.h
    {
        ...
        {   // Move down the tree and add a new leaf:
            ...
            bool is_there = false;
            /* STUDENT WORK */
            while(!is_there)//in order to loop through the whole tree
            {
                if (cursor->data() < entry)//entry is greater
                {
                    if (cursor->right() == NULL)//if at the end
                    {
                        cursor->right() = new binary_tree_node<Item>(entry);//new node
                        is_there = true;//leave loop
                    }
                    cursor = cursor->right();//go right
                }
                else if (cursor->data() > entry)//entry is smaller
                {
                    if (cursor->left() == NULL)//if at the end
                    {
                        cursor->left() = new binary_tree_node<Item>(entry);//new node
                        is_there =  true;//leave loop
                    }
                    cursor = cursor->left();//go left
                }
                else if (cursor->data() == entry)//entry is equal
                {
                    if (cursor->left() == NULL)//if at the end
                    {
                        cursor->left() = new binary_tree_node<Item>(entry);//new node
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        binary_tree_node<Item> *cursor;
        cursor = root_ptr;
        if (root_ptr == NULL)//an empty list
            return 0;
        else if (root_ptr != NULL)//not an empty list
        {
            while (cursor != NULL)//not end of the list
            {
                if (cursor->data() < target)//target is greater
                    cursor = cursor->right();
                else if (cursor->data() > target)//target is smaller
                    cursor = cursor->left();
                else//target is equal
                {
                    answer++;//increase count
                    cursor = cursor->left();//continue checking on left
                }
            }
        }
        return answer;
